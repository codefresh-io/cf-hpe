...
function createServer(session, server) {
  const uri = Util.format('%s/ci_servers/', workspaceUri(session));
  const data = {
...
function createPipeline(session, pipeline) {
  const uri = Util.format('%s/pipelines/', workspaceUri(session));
...
function reportPipelineStepStatus(session, stepStatus) {
  const uri = Util.format('%s/analytics/ci/builds/', workspaceUri(session));
...
function reportPipelineTestResults(session, testResult) {
  const uri = Util.format('%s/test-results/', workspaceUri(session));
...
export default {
  ...
  createServer,
  createPipeline,
  reportPipelineStepStatus,
  reportPipelineTestResults,
};
