...
Build.buildsFromFirebase = () => Build
  .openBuildLogsRef()
...
Build.openBuildLogsRef = () => Rx.Observable
...
Build.findAccount = (buildLogSnapshot) => Rx.Observable
...
Build.findService = (buildLogSnapshot) => Rx.Observable
...
Build.findBuild = (buildLogSnapshot) => Rx.Observable
  .fromPromise(() => Model.Build.findOne(
    { progress_id: Model.toObjectId(buildLogSnapshot.key()) }))
  .filter(build => {
    if (!build) {
