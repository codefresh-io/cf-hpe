...
class Hpe {
  ...
  static createServer(session, server) {
    const uri = Util.format('%s/ci_servers/', Hpe.workspaceUri());
    const data = {
  ...
  static createPipeline(session, pipeline) {
    const uri = Util.format('%s/pipelines/', Hpe.workspaceUri());
  ...
  static reportPipelineStepStatus(session, stepStatus) {
    const uri = Util.format('%s/analytics/ci/builds/', Hpe.workspaceUri());
