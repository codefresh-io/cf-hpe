...
MochaJsonStreamReporter.create = function (buildStepObservable, buildSession) {
  return buildStepObservable.filter(function (step) {
    ...
    return _buildStep.BuildStep.childStepLogs(step).filter(_ramda2.default.test(/^\["(pass|fail)",{"title":"[\s\S]+","fullTitle":"[\s\S]+",[\s\S]+}]/m)).map(JSON.parse).map(function (testResult) {
